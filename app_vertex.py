...
# ── Vertex AI init (same config as your working test_vertex.py) ───────────────
vertexai.init(
    project="project-356372ac-d01e-498f-8e9",
...
def extract_invoice_fields(file_path, invoice_source="electronic", is_dmh=False):
    try:
        # ===============================
        ...
        for img in images:
            # Convert PIL Image → PNG bytes → Vertex AI Part
        ...
        # ===============================================================
        # 🔥 FEATURE 1 — Deduplicate replica pages (DMH ONLY)
        #
        # Invoices often come as Original + Duplicate + Triplicate copies
        # inside the same PDF.  If multiple pages share the same PO number
        # (case-insensitive, stripped) they are considered replicas of the
        # same invoice.  Keep only the FIRST occurrence so that items,
        # quantities and amounts are never double-counted.
        #
        # This logic runs ONLY for DMH users (is_dmh=True).
        # ===============================================================
        def _normalise_po(po_val):
            """Return a stripped, upper-case string or None."""
            if not po_val:
                return None
            return str(po_val).strip().upper()

        if is_dmh:
            seen_po_numbers = set()
            deduplicated_pages = []
            for page in all_pages_data:
                po = _normalise_po(page.get("PO_number"))
                if po:
                    if po not in seen_po_numbers:
                        seen_po_numbers.add(po)
                        deduplicated_pages.append(page)
                    else:
                        print(f"\U0001f501 DMH: Duplicate replica skipped — PO number already seen: {po}")
                else:
                    # Pages without a PO number are kept during dedup;
                    # the gate below will reject the whole invoice if none found.
                    deduplicated_pages.append(page)
            all_pages_data = deduplicated_pages

        # ===============================================================
        # 🔥 FEATURE 2 — PO number required gate (DMH ONLY)
        #
        # After deduplication, check whether at least one page carries a
        # non-empty PO number.  If not, reject the whole invoice and
        # return a clear error — do NOT proceed with extraction.
        #
        # This logic runs ONLY for DMH users (is_dmh=True).
        # Regular users are not affected and their invoices are processed
        # regardless of whether a PO number is present.
        # ===============================================================
        if is_dmh:
            invoice_po = None
            for page in all_pages_data:
            ...
            if not invoice_po:
                conn.close()
                print("\u274c DMH: Invoice rejected — no PO number found.")
                return {
                    "error": "Invoice rejected: PO number is missing. "
                             "Only invoices with a valid PO number are processed for DMH."
                }

            print(f"\u2705 DMH: PO number confirmed: {invoice_po}")
        # ===============================================================
...
def process_invoice():
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            # Look up the caller's role via their API key for DMH-only features
            _api_key = request.headers.get("X-API-Key")
            _conn2   = get_db_connection()
            _cur2    = _conn2.cursor(dictionary=True)
            _cur2.execute(
                "SELECT u.role FROM users u JOIN api_keys ak ON u.id = ak.user_id WHERE ak.api_key = %s",
                (_api_key,)
            )
            _role_row = _cur2.fetchone()
            _cur2.close()
            _conn2.close()
            _is_dmh = (_role_row["role"] == "dmh") if _role_row else False

            extracted_data = extract_invoice_fields(filepath, invoice_source, is_dmh=_is_dmh)
...
def upload_invoice():
    ...
    is_dmh         = (role == "dmh")  # ✅ DMH-only PO features
    extracted_data = extract_invoice_fields(file_path, is_dmh=is_dmh)
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if doc_type == "invoice":
                ...
                is_dmh         = (role == "dmh")  # ✅ DMH-only PO features
                extracted_data = extract_invoice_fields(file_path, invoice_source, is_dmh=is_dmh)
...
def api_extract_invoice():
    ...
    file.save(file_path)
    # Look up the caller's role to apply DMH-only features
    cursor.execute("SELECT role FROM users WHERE id = %s", (api_key_record["user_id"],))
    caller_role_row = cursor.fetchone()
    caller_is_dmh   = (caller_role_row["role"] == "dmh") if caller_role_row else False
    invoice_data = extract_invoice_fields(file_path, is_dmh=caller_is_dmh)
    if invoice_data:
        sql_update = (
