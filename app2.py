...
#             flash("Login details missing", "danger")
#             return redirect(url_for("login"))

#         user = get_user_from_db(login_input, password)
...
def upload_invoice():
    ...
    prefix = "M" if role == "user" else "E"
    invoice_id = generate_invoice_id(prefix)

    cursor = db.cursor()
    cursor.execute(
        """
        INSERT INTO extraction_history
        (invoice_id, user_id, timestamp, image_name, pages_extracted, extraction_type, extracted_data)
        VALUES (%s, %s, NOW(), %s, %s, %s, %s)
        """,
        (invoice_id, user_id, image_name, pages_extracted, extraction_type, extracted_json),
    )
...
# def generate_invoice_id(prefix):
...
#     if last_id:
#         last_number = int(last_id[0][-3:])
#         new_number = last_number + 1
#     else:
#         new_number = 1

#     return f"{prefix}{today}{new_number:03d}"
