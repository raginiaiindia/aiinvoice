...
2. If quantity is written in a combined format such as:
   ...
   → Set free_item_yn = "yes" for that item.
...
#  """ 
        ...
        # ==================================================
        #  ADD TAX CALCULATION BLOCK **HERE**
        # ==================================================
        for page in all_pages_data:
            for item in page.get("items", []):
                taxable = item.get("total_price")

                if item.get("cgst_rate") and not item.get("cgst_amount") and taxable:
                    item["cgst_amount"] = round(taxable * item["cgst_rate"] / 100, 2)

                if item.get("sgst_rate") and not item.get("sgst_amount") and taxable:
                    item["sgst_amount"] = round(taxable * item["sgst_rate"] / 100, 2)

                if item.get("igst_rate") and not item.get("igst_amount") and taxable:
                    item["igst_amount"] = round(taxable * item["igst_rate"] / 100, 2)

                if not item.get("GST_AMT"):
                    item["GST_AMT"] = round(
                    ...
                    # ===============================
                    # 🔹 CALCULATE HEADER GST TOTALS FROM ITEMS
                    # ===============================
        total_cgst_amount = 0
        total_sgst_amount = 0
        total_igst_amount = 0
    ...
    #     #ADD THIS BLOCK **HERE** (AFTER LOOP, BEFORE RETURN)
       

    #     final_header = {}

    #     # initialize all header fields as None
    #     for field in HEADER_FIELDS:
    #         final_header[field] = None

    #     # merge header values from all pages
    #     for page in all_pages_data:
    #         for field in HEADER_FIELDS:
    #             value = page.get(field)
